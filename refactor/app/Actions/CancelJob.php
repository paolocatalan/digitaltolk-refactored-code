...
class CancelJob
{
    ...
    public function __construct(NotificationServices $notification, JobServices $jobServices)
    {
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->notification->isNeedToSendPush($translator->id)) {
                    $users_array = array($translator);
                    $this->notification->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->notification->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->notification->isNeedToSendPush($customer->id)) {
                        $users_array = array($customer);
                        $this->notification->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->notification->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
